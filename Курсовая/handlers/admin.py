...
from keyboards.admin_kb import admin_kb, stop_load, ganre_kb, confirm_kb
...
async def load(message: types.Message):
    await message.answer("Отправьте видео фильма", reply_markup=stop_load)
...
async def load_video(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        data["video"] = video
    await FSMload.next()
    await message.answer("Введите НАЗВАНИЕ фильма", reply_markup=stop_load)
...
async def load_name(message: types.Message, state: FSMContext):
    ...
    await FSMload.next()
    await message.answer("Введите Жанр фильма", reply_markup=ganre_kb)
...
async def load_ganre(callbaack: types.CallbackQuery, state: FSMContext):
    ...
    await FSMload.next()
    await callbaack.message.answer("Введите Описание фильма", reply_markup=stop_load)
...
async def load_desc(message: types.Message, state: FSMContext):
    ...
    await FSMload.next()
    await message.answer("Введите год производства ", reply_markup=stop_load)
...
async def load_date(message: types.Message, state: FSMContext):
    ...
    await FSMload.next()
    await message.answer("Введите страну производства ", reply_markup=stop_load)
...
async def load_country(message: types.Message, state: FSMContext):
    ...
    await FSMload.next()
    await message.answer("Введите режисёров фильма", reply_markup=stop_load)
...
async def load_producer(message: types.Message, state: FSMContext):
    ...
    await FSMload.next()
    await message.answer("Введите Актёров фильма", reply_markup=stop_load)
...
async def load_actors(message: types.Message, state: FSMContext):
    ...
    await message.answer("Подтвердите загрузку : ", reply_markup=confirm_kb)
    await FSMload.next()
...
async def load_confirm(message: types.Message, state: FSMContext):
    if message.text.startswith("Подтвердить"):
        ...
        await message.answer("Фильма отправлен на загрузку", reply_markup=admin_kb)
    else:
        await message.answer("Загурзка фильма не осуществлена", reply_markup=admin_kb)
    await state.finish()
