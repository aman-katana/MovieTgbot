...
def movies_kb(info, genre, page=1):
    ...
    for i in list(info):
        id = i[0]
        d = dict(i[1])
        movie_btn.add(InlineKeyboardButton(text=f"{d['name']}", callback_data=f"show_movie_{id}"))
...
def search_kb(info, query, page=1):
    ...
    for i in list(info):
        id = i[0]
        d = dict(i[1])
        search_btn.add(InlineKeyboardButton(text=f"{d['name']}", callback_data=f"show_movie_{id}"))
...
def pop_movies_kb(info):
    ...
    for i in list(info):
        id = i[0]
        d = dict(i[1])
        movie_btn.add(InlineKeyboardButton(text=f"{d['name']}", callback_data=f"show_pop_{id}"))
...
def liked_movies_kb(info):
    ...
    for i in list(info):
        id = i[0]
        d = dict(i[1])
        movie_btn.add(InlineKeyboardButton(text=f"{d['name']}", callback_data=f"show_liked_{id}"))
